...
export class GameMapComponent implements OnInit, OnDestroy {
      ...
      if (
        ...
        this.viewer.camera.flyTo({
          destination: Cesium.Cartesian3.fromRadians(
            ...
            posWithHeight.latitude,
            posWithHeight.height
          ),
          complete: () => {
  ...
  private startFirstPersonMode(
    player: PlayerFields.Fragment,
    initCharacter = true
  ) {
    if (initCharacter) {
      this.character.initCharacter({
        ...
        location: player.enteringBuildingPosition
          ? this.utils.getPosition(player.enteringBuildingPosition)
          : this.utils.getPosition(player.currentLocation.location),
        heading: player.currentLocation.heading,
        pitch: GameMapComponent.DEFAULT_PITCH,
        state:
          player.state === "DEAD" ? MeModelState.DEAD : MeModelState.WALKING,
        team: player.team,
        isCrawling: false,
        isShooting: false,
        isFlying: false,
  ...
  private overviewSettings() {
    ...
    this.viewer.camera.flyTo({
      destination: GameMapComponent.DEFAULT_START_LOCATION
    });
  }
  ...
  preRenderHandler() {
    ...
    const playerHeadCart = Cesium.Cartographic.fromCartesian(
      this.character.location
    );
    ...
    } else playerHeadCart.height += 4.4;
